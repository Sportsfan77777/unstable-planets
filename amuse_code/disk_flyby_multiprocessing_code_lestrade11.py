...
def initialize_kepler(stars, planetesimals, converter, file_redir, center = 0):
...
def integrate_disk_flyby(stars, planetesimals, t_end, t_peri, nb_end, n_steps,
  ...
  planetesimals_gravity = initialize_kepler(stars, planetesimals, converter, file_redir, center = 0)
  ...
  stars_gravity = initialize_huayno(stars, converter, huayno_eta)
  planetesimals_gravity = initialize_kepler(stars, planetesimals, converter, file_redir, center = 1)
...
def new_option_parser():
  ...
  result.add_option("--n_disk", 
                    dest="n_disk", type="int", default=500,
  ...
  result.add_option("--power", 
                    dest="power", type="float", default=1.0, 
  ...
  result.add_option("--num_iter",
                    dest="num_iter", type="int", default=7,
                    help="number of different random seeds [%default]")
...
if __name__ in ('__main__', '__plot__'):
  ...
  for i in range(num_sim):
      print "Process", i
      o_tmp = copy.deepcopy(o)
      o_tmp.seed = random.randint(1, 1000000)
      o_tmp.iter_i = i # Iteration Number 'i'

      r, p = handle_input(o_tmp, arguments)
      oss.append(r)
      processes.append(p)

  print o

  o.snap_base = o.snap_dir # slight modification (added 'snap_base')
  ...
  for i in range(num_sim):
      # get final counts
      count = get_final_count(oss[i])
      final_counts.append(count)

  min_count = min(final_counts)
  max_count = max(final_counts)

  chosen = [] # 'grand' input for factory
  filter_min = False
  filter_max = False
  for i, count in enumerate(final_counts):
      if (count == min_count) and not filter_min:
          filter_min = True
      elif (count == max_count) and not filter_max:
          filter_max = True
      else:
          chosen.append(i)
  ...
  chosen_sum = 0
  for ch in chosen:
      chosen_sum += final_counts[ch]

  mean = chosen_sum * 100.0 / len(chosen) / o.n_disk # mean (as a %)
  mean = round(mean, 1)
  std_dev = numpy.std(numpy.array(final_counts)) # <--- add this to the dictionary
  ...
  ## set up the new 'grand' directory NOW!!!!!!!!!!!!!!!!!!!!!!!!
  print o # <--- Also, SAVE 'o'

  # Initialize 'grand' directory  
  if o.num_format == 3:
      number = '%03d' % int(o.sim_number) # Old Format
  else:
      number = '%06d' % int(o.sim_number) # New Format (will fail if count = 4 or 5, which is bad)

  snap_dir = o.snap_dir + number

  ps.mkdir(snap_dir, safety = True)

  results = {} ### how to make this like 'o' ????
  results['mean'] = mean
  results['std_dev'] = std_dev

  # Write 'results' dictionary into file (using Pickle)
  pickle_f = open(snap_dir + "/results.p", "wb")
  pickle.dump(results, pickle_f)
  pickle_f.close()

  grand_plot_all(o, chosen) # Default 'o'

  # Change name format
  # Add option in factory for 'iter'

  # Instead of plotting let us count the final particles (in parallel, lol) <--change rest to be in parallel also?
  # Then, filter 2 of 7
  # Then, combine output of 5 of 7 into one big output file
  # Then, do plot_all() for the big output file through one big factory

  print " *** done *** "

  final_time = time.time()
  dt = final_time - start_time
  print "dt=", dt, "s =", dt/60.0, "min"
