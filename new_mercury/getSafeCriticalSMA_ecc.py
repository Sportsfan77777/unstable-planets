...
for ith, inc in enumerate(inc_s):
    for j,ecc in enumerate(ecc_s):
        for k,u in enumerate(mass_ratios):
            ...
            crit_sma = sm_axes[-1] + 90.0
            found = False
            for (sma, s0, s1) in zip((sm_axes)[::-1], (stable_arrays[0])[::-1], (stable_arrays[1])[::-1]):
                 if not found:
                    if s0 and s1:
                        crit_sma = sma
                    else:
...
pickle_f = open(("%s_safe_critical_sma_ecc_tables.p" % (dir_base)), "wb")
pickle.dump(critical_sma_tables, pickle_f)
pickle_f.close()

pickle_f = open(("%s_safe_critical_sma_ecc_list.p" % (dir_base)), "wb")
pickle.dump(critical_sma_list, pickle_f)
pickle_f.close()
            
# Pretty Print Table and List

tables_fn = "%s_safe_critical_sma_ecc_tables.txt" % (dir_base)
list_fn = "%s_safe_critical_sma_ecc_list.txt" % (dir_base)
