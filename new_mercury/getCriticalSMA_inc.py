...
for ith, inc in enumerate(inc_s):
    for j,ecc in enumerate(ecc_s):
        for k,u in enumerate(mass_ratios):
            ...
            second_eject_t = STABLE_VALUE
            third_eject_t = STABLE_VALUE
            for (sma, s0, s1, min_eject_t0, min_eject_t1) in zip(sm_axes, stable_arrays[0], stable_arrays[1], min_eject[0], min_eject[1]):
                third_eject_t = second_eject_t
                second_eject_t = crit_eject_t
                crit_eject_t = min(min_eject_t0, min_eject_t1)
                if s0 and s1:
                     crit_sma = sma
                     # Take minimum of 3 previous minimum eject times
                     crit_eject_t = min(crit_eject_t, second_eject_t, third_eject_t)
                     break
...
pickle_f = open(("%s_critical_sma_inc_tables.p" % (dir_base)), "wb")
pickle.dump(critical_sma_tables, pickle_f)
pickle_f.close()

pickle_f = open(("%s_critical_sma_inc_list.p" % (dir_base)), "wb")
pickle.dump(critical_sma_list, pickle_f)
pickle_f.close()

pickle_f = open(("%s_critical_sma_inc_eject_tables.p" % (dir_base)), "wb")
pickle.dump(critical_sma_eject_time_tables, pickle_f)
pickle_f.close()

pickle_f = open(("%s_critical_sma_inc_eject_list.p" % (dir_base)), "wb")
pickle.dump(critical_sma_eject_time_list, pickle_f)
pickle_f.close()
            
# Pretty Print Table and List

tables_fn = "%s_critical_sma_inc_tables.txt" % (dir_base)
list_fn = "%s_critical_sma_inc_list.txt" % (dir_base)

eject_tables_fn = "%s_critical_sma_inc_eject_tables.txt" % (dir_base)
eject_list_fn = "%s_critical_sma_inc_eject_list.txt" % (dir_base)
