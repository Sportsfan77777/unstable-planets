...
def is_librating(ecc, inc):
    a = -0.714
    b = 0.102
    c = 0.526
    d = -0.394
    i_crit = 0.5 + a * ecc + b * ecc**2 + c * ecc**3 + d * ecc**4
    i_crit = 180.0 * i_crit # convert to degrees
    #print ecc, inc, i_crit
    return (inc > i_crit)
...
for ith, inc in enumerate(inc_s):
    for j,ecc in enumerate(ecc_s):
        for k,u in enumerate(mass_ratios):
            ...
            librating = is_librating(ecc, inc)
            if not librating:
               ### Filter these out when setting up histogram ###
               crit_sma = -1
               crit_eject_t = -1
...
pickle_f = open(("%s_safe_librating_critical_sma_inc_tables.p" % (dir_base)), "wb")
pickle.dump(critical_sma_tables, pickle_f)
pickle_f.close()

pickle_f = open(("%s_safe_librating_critical_sma_inc_list.p" % (dir_base)), "wb")
pickle.dump(critical_sma_list, pickle_f)
pickle_f.close()

pickle_f = open(("%s_safe_librating_critical_sma_inc_eject_tables.p" % (dir_base)), "wb")
pickle.dump(critical_sma_eject_time_tables, pickle_f)
pickle_f.close()

pickle_f = open(("%s_safe_librating_critical_sma_inc_eject_list.p" % (dir_base)), "wb")
pickle.dump(critical_sma_eject_time_list, pickle_f)
pickle_f.close()
            
# Pretty Print Table and List

tables_fn = "%s_safe_librating_critical_sma_inc_tables.txt" % (dir_base)
list_fn = "%s_safe_librating_critical_sma_inc_list.txt" % (dir_base)

eject_tables_fn = "%s_safe_librating_critical_sma_inc_eject_tables.txt" % (dir_base)
eject_list_fn = "%s_safe_librating_critical_sma_inc_eject_list.txt" % (dir_base)
